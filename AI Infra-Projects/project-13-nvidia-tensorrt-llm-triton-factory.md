...
**Difficulty:** ★★★★☆ | **Time:** 3–4 weekends | **Cost:** ~$15–35 (g5/g6 spot; engines/checkpoints build on the same GPU that serves them; NGC registration is free; NIM developer tier is free)

*NVIDIA doesn't just sell GPUs — it ships the software factory that runs them: **TensorRT-LLM** (the LLM compiler/runtime), **Triton Inference Server** (the general-purpose server, now branded **Dynamo-Triton**), **NIM microservices** (the factory prepackaged and licensed), and the **NGC** catalog they all pull from. In this project you build the optimization factory yourself — quantize, serve, benchmark, canary — then deploy NVIDIA's prepackaged version of the same factory (NIM), compare both honestly against your open-source vLLM baseline, and Nsight-profile every rung so the speedups are attributed, not asserted.*
...
NGC catalog ──► optimized containers & models (Triton, TensorRT-LLM, NIM)
     ...
     │
     ├── NIM ──► TRT-LLM + serving + OpenAI API prepackaged as ONE licensed microservice  (Phase 4, here)
     │
...
```
 HF weights ─▶ [1] quantize (Model Optimizer: INT4-AWQ | FP8) → quantized checkpoint
            ...
            ─▶ [2b] Lane B (legacy, ≤1.2.x): convert_checkpoint → trtllm-build → engine (per GPU arch)
            ─▶ [3] push to S3 "artifact registry" (s3://engines/llama31-8b/awq/sm86/v3/)
            ─▶ [4] serve: trtllm-serve Deployment (A) | Triton ensemble w/ in-flight batching (B) | NIM (vendor)
...
Record three things from the timeline: the **kernel mix** of a decode step (GEMM vs attention vs elementwise/memcpy), **gaps** between kernels (launch overhead, CPU-side stalls), and achieved tensor throughput vs the hardware peak. For that last one, use the *dense* peak: **A10G = 70 TFLOPS dense FP16/BF16** (official AWS/NVIDIA datasheet; 140 TF with 2:4 sparsity — the ~125 TFLOPS figure that floats around is the NVIDIA *A10*'s dense number, a different SKU that third-party spec pages and even AWS's own G5 page conflate with the A10G); **L4 = 121 dense** (NVIDIA's L4 sheet prints 242 sparse-first with a "one-half lower without sparsity" footnote); A100 = 312 dense; T4 = 65 FP16 (Turing has no BF16 tensor path and no sparsity mode at all). Never put a 2:4-sparsity figure in a utilization denominator — no standard inference workload uses structured-sparse weights, and the PaLM/MFU convention is the dense peak.

This phase is deliberately short: the full CUDA treatment — Nsight Compute per-kernel analysis, roofline placement, occupancy, writing your own kernels — is [P16](project-16-cuda-gpu-performance-engineering.md), recommended early in the track (right after P1) for exactly this reason. Here you only need the baseline trace and the decode-step kernel mix written down, because **Phase 3 re-profiles the optimized paths against this baseline** to attribute where the speedup actually comes from.
...
Two Triton primitives to be able to explain: **dynamic batching** (server-side request coalescing — the general-purpose ancestor of LLM continuous batching; measure its effect, throughput at fixed p95 vs `max_queue_delay`) and **concurrent model execution** (`instance_group` count — N copies of a model sharing one GPU, the crude ancestor of [P08](project-08-fractional-gpu-dra-multitenancy.md)'s fractional-GPU work). Wire `nv_inference_queue_duration_us` / `nv_gpu_utilization` into your [P12](project-12-ai-fleet-sre-finops-aiops.md) Grafana.
...
Fix these before you record a single row, and state them at the top of the results doc: **fixed ISL/OSL** (~800 in / ~200 out, identical across every path — change the shape and you change the winner); **warm-up then measure** (discard the first N requests: engine/CUDA-graph warm-up, KV allocator settling); **sweep concurrency** 1/4/16/64 (one number is a marketing claim, a curve is a result) reporting **TTFT and TPOT separately** (a single "latency" figure hides which one you broke); **p50 and p99, not means**; **load generator off the GPU node**; and the **same client for every server** — they all speak OpenAI-compatible, so keep it constant.
...
Run it against all OpenAI-compatible frontends (vLLM, `trtllm-serve`, Triton, and NIM in Phase 4) so the client is identical. Produce THE table: **FP16-vLLM vs FP16-TRTLLM vs AWQ-TRTLLM (vs NIM)** × concurrency → TTFT p50/p99, TPOT p50/p99, tokens/s/GPU, plus your quality-eval delta, $/1M-tokens (from [P12](project-12-ai-fleet-sre-finops-aiops.md)'s pricing), and two columns most benchmarks omit:
...
Then the honest paragraph: where vLLM wins (flexibility, day-0 model support, no per-arch builds), where TRT-LLM wins (peak throughput/latency on NVIDIA, quantization maturity). Balanced judgment > fanboyism. **That trade-off table is the deliverable** — it's the decision a serving team makes and the reason both stacks exist.
...
Model Analyzer sweeps batch size / instance count / concurrency to find the optimal Triton config under a latency budget and emits the Pareto frontier of throughput vs latency. Deliverable: the analyzer's report + your interpretation, and the general lesson — this is how you *actually* size any inference deployment, Triton or not (connect to [P10](project-10-fault-tolerant-training-goodput.md)'s goodput framing).

### Attribute, don't assert (Nsight re-profile)

Re-run the exact Phase 0 `nsys` capture against the AWQ engine under identical aiperf load. Diff against the baseline: which kernels disappeared (fused), how the GEMM-vs-attention split shifted, what happened to inter-kernel gaps, and the new achieved-vs-70-TFLOPS ratio. Your Phase 3 table now has a *cause* attached to every speedup number — "X% faster because the quantized fused-attention path cut kernel count per token from A to B" is a sentence very few candidates can back with their own traces. Full methodology (Nsight Compute, roofline, occupancy) in [P16](project-16-cuda-gpu-performance-engineering.md).

## 7. Phase 4 — NIM: the vendor's factory, prepackaged

**NIM microservices** (NVIDIA now brands them that way rather than expanding the acronym) are the same factory you just built — TensorRT-LLM optimization + serving + an OpenAI-compatible API — prebuilt, per-GPU-profiled, and NVIDIA-supported, pulled from NGC and deployed in minutes. Licensing, precisely (as of July 2026): NIM containers are governed by the **NVIDIA AI Enterprise** license; **Developer Program members get free access for research/dev/test (self-hosting up to 16 GPUs)** — so you *can* run this phase hands-on for free — while **production requires an AI Enterprise subscription** (~$4,500/GPU/yr list, or ~$1/GPU/hr via cloud marketplaces; 90-day evaluation available). If you can't get entitlement at all, do this phase as an architecture + pricing analysis from the NIM docs — still a strong artifact, just say so in the write-up.

Deploy via the **NIM Operator** (v3.1.1 as of July 2026; operator source Apache-2.0, the NIM artifacts it pulls are AI-Enterprise-licensed), whose CRDs are the vendor mirror of everything you hand-rolled: **NIMCache** (model download/caching from NGC to shared storage — the vendor's version of your S3 artifact registry), **NIMService** (a deployment with autoscaling, ingress/Gateway API, monitoring), **NIMPipeline** (grouped services), and — the punchline — **NIMBuild**, which builds optimized TensorRT-LLM engines from model profiles: NVIDIA productized your Phases 1–2.

```yaml
# 1) cache the model + its optimized profiles once (check exact schema on your operator release)
...
| Custom/fine-tuned models | LoRA + NIMBuild paths, within the profile system | anything you can quantize | anything |
| Support | enterprise SLA | you are the support | community |
| $/1M tokens (measured) | … | … | … |

State when each wins — every AI-factory-adjacent team weighs exactly this, and having *measured* both sides (latency **and** operational effort **and** license cost) is what makes the doc credible. The honest framing for the vendor path: you are buying back engineering weekends and an SLA, and paying in dollars and lock-in.
...
Rollout: Argo Rollouts canary (10 %→50 %→100 %) with an **AnalysisTemplate** querying Prometheus for canary-vs-stable p99 TTFT and error rate; auto-rollback on breach. This closes the MLOps loop — but with GPU-native gates, which is the differentiator.
...
- [ ] Both lanes run: `trtllm-serve` (PyTorch backend) and a legacy AWQ engine served via Triton in-flight batching, streaming end-to-end — and you can explain why Lane B is being retired in 1.3.
...
- [ ] Five-way benchmark table (vLLM / TRT-LLM FP16 serve / TRT-LLM FP16 engine / TRT-LLM AWQ / NIM) + $/1M-token economics, measured with AIPerf under a stated methodology; Model Analyzer Pareto report interpreted.
...
**Whiteboard-ready:** Triton backends, dynamic batching vs in-flight batching, and where the model repository sits; what TensorRT-LLM compilation actually does and why it costs you a per-arch matrix; why `trtllm-build` is going away and what replaces it; what NIM bundles and what it costs; how you size an inference deployment (Model Analyzer sweep + AIPerf curve, not one number); when a factory standardizes on NVIDIA's stack vs rolls its own.
...
Stop the GPU runner ASG at 0; engines/checkpoints cached in S3 (and the NIMCache PVC) cost pennies — never rebuild. Serving pods on session economics; drain GPU nodes when done. Keep the engines, traces, and reports.

## 11. Extensions

- Multi-GPU engine (`--tp_size 2`) on g5.12xlarge to learn tensor-parallel serving — connects to [P10](project-10-fault-tolerant-training-goodput.md)'s fabric lessons and [P15](project-15-nvidia-networking-nccl-cluster-validation.md)'s NCCL validation.
- Speculative decoding (draft model) in TRT-LLM; Medusa/EAGLE heads comparison memo.
- ONNX→TensorRT path for a non-LLM model (embedder) through the same Triton repo — shows breadth.
- Put NIM/Triton behind [P09](project-09-disaggregated-inference-llm-d.md)'s inference gateway — vendor serving + open routing, best of both.
...
- **Close the customize→evaluate→serve loop:** the same NIM Operator also manages the NeMo microservice CRDs (Customizer for LoRA/SFT/DPO/GRPO on Volcano-scheduled K8s, Evaluator, Guardrails, Data/Entity Store). Fine-tune with Customizer, evaluate, then serve the adapter through NIM — and note the NeMo *framework* has been modularized (Megatron-Bridge, NeMo AutoModel, NeMo RL, launched by NeMo-Run), so be precise about which "NeMo" you mean.
- The datacenter rung — disaggregated prefill/decode, KV-aware routing, KVBM/NIXL, DynamoGraphDeployments — is its own project: [P14](project-14-nvidia-dynamo-inference-os.md). The three-way write-up (`docs/disagg-comparison.md`: vLLM-native disagg vs llm-d ([P09](project-09-disaggregated-inference-llm-d.md)) vs Dynamo) lands there, built on the engines you produced here.

## 📣 Build in public

- **LinkedIn:** post THE table — five serving paths × concurrency with TTFT/TPOT p99, tokens/s/GPU, quality delta, and $/1M tokens on the same ~$1/hr spot GPU, NIM row included with its license cost stated — under the hook "quantization without a quality gate is malpractice," and let the AWQ-vs-FP16 quality-delta number carry the argument.
- **X/Twitter thread:** the tooling-shift story with receipts — "`trtllm-build` is legacy and gone in TRT-LLM 1.3; genai-perf is deprecated in favor of AIPerf; Model Analyzer is *not* dead, and it never did genai-perf's job" — illustrated by your before/after Nsight kernel-mix screenshots and the measured X% throughput gap between the PyTorch-backend and engine lanes.
- **YouTube:** live-fire the factory end-to-end — push a `build.yaml` change, watch CI quantize→build→aiperf-gate, then inject a p99 TTFT regression and let the Argo Rollouts AnalysisTemplate auto-rollback on camera with the Prometheus canary-vs-stable graph on screen; close by deploying the same model as a NIMService in 90 seconds and showing what it cost you.
